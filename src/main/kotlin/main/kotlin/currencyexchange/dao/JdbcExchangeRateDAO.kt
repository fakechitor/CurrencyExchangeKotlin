    ...
    private fun getByCode(code: String, connection: Connection?): ExchangeRateDTO {
        ...
        connection?.prepareStatement(sql)?.use { ps ->
            ...
            ps.executeQuery().use { rs ->
                if (rs.next()) {
                    val id = rs.getInt(1)
                    val baseCur = jdbcCurrencyDAO.getByCode(rs.getString(2))
                    val targetCur = jdbcCurrencyDAO.getByCode(rs.getString(3))
        ...
        connector.getConnection().use { conn ->
            conn!!.createStatement().use { stmt ->
                ...
                while (rs.next()) {
                    ...
                    val baseCurrency = jdbcCurrencyDAO.getById(rs.getInt(2))
                    val targetCurrency = jdbcCurrencyDAO.getById(rs.getInt(3))
